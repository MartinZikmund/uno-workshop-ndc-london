...
We also need to add the `AppThemeService.cs` file from `references` to the `SimpleCalculator` project. This service needs to be initialized for the current window. We can do so in `AppStart.cs` right after the window is created:
